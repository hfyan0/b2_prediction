...
# CONFIG_HIST=/home/qy/Dropbox/nirvana/b2_prediction/config_test.ini
...
###################################################
ls|grep config|grep ini|head -n 1|xargs cat|grep CoefFolder|awk -F= '{print $2}'|xargs mkdir -p
ls|grep config|grep ini|head -n 1|xargs cat|grep ForecastFolder|awk -F= '{print $2}'|xargs mkdir -p
